...
import "../RegistrationPages/ChooseCuisines.css";
...
const EditCuisines = () => {
    ...
    const translations = useContext(TranslationContext).translations.ProfilePages.EditCuisinesJourney;
    const cuisinesData = [
    ...
    const handleContinue = () => {
        if (selectedCuisines.length >= 1) {
            ...
            newAtts["CuisineCuisine"] = selectedCuisineNames;
            ...
            fetch(`${process.env.REACT_APP_API_PATH}/users/${userID}`, {
                ...
                .then(data => {
                    ...
                    navigate("/profile"); // Proceed to the next step
                })
                .catch(error => {
    ...
    return (
        <div className="container_choose_cuisine">
            ...
            <h2 className="cuisine-heading_choose create-heading">{translations["edit_cuisine_preferences"]}</h2>
            {showError ? <ErrorPopUp text={translations["choose_cuisine_error"]}/> : null}
            <p className="cuisine-text_choose">{translations["click_cuisines"]}</p>
            ...
            <input className="submit" type="submit" value={translations["save"]} onClick={handleContinue}/>
        </div>
    );
}
};
